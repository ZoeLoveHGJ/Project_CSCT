...
RESULTS_BASE_DIR = "exp_final_2_results_vs_estimator"
ALGO_STYLES = {
    'CSCT': {
        'color': 'red', 'linestyle': '-', 'marker': 's', 'label': 'CSCT'
    },
    'CSCT_Est': {
        'color': 'blue', 'linestyle': '--', 'marker': '^', 'label': 'CSCT-Est'
    }
}
DYNAMIC_PROFILE = [
...
def plot_dynamic_throughput(throughput_data: pd.DataFrame, profile: List[Tuple[float, int]]):
    ...
    save_path = os.path.join(
        RESULTS_BASE_DIR, "chart_dynamic_throughput_comparison_enlarged.pdf")
