...
def save_results(results: Dict, output_dir: str):
    ...
    csct_est_df = pd.DataFrame(results['CSCT_Est']['time_series_data'])

    throughput_df = pd.DataFrame({
        'time_s': csct_df['time_ms'] / 1000,
        'CSCT': csct_df['interval_throughput_tps'],
        'CSCT_Est': csct_est_df['interval_throughput_tps']
    })
    ...
    backlog_df = pd.DataFrame({
        ...
        'CSCT_Est': csct_est_df['unidentified_count_mean']
    })
...
def main():
    ...
    algorithms_to_test = {
        ...
        "CSCT_Est": {
            "class": CSCT_Est,
            "config": {
                'initial_q': 11.0, 'priority_weight_size': 1, 'priority_weight_depth': 0.0001,
            }
        }
    }
    ...
    output_directory = "exp_final_2_results_vs_estimator"
