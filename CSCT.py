...
from Framework import ReliableAlgorithmBase, Tag, AlgorithmStepResult, CONSTANTS, calculate_time_delta
...
class CSCT(ReliableAlgorithmBase):
    def __init__(self, algorithm_specific_config: Dict):
        super().__init__(algorithm_specific_config)
        self.ccq: list = []
        self.next_context_id: int = 0
        self.q_value: float = algorithm_specific_config.get('initial_q', 9.0)
        self.estimated_size_per_context = algorithm_specific_config.get(
    ...
    def _phase_B_resolve(self, active_tags: List[Tag], current_time_us: float) -> Optional[AlgorithmStepResult]:
        ...
        context.tags = self.get_actual_responders(context.tags)
