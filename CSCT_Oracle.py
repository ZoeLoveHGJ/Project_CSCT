...
class CSCT_Oracle(MobileAlgorithmInterface):
    ...
    def _process_frame_results(self):
        ...
        for slot_index, tags in self._transient_collision_tags.items():
            ...
            actual_size_per_context = float(len(tags))

            new_context = CollisionContext(
                context_id=self.next_context_id,
                tags=tags,

                estimated_size=actual_size_per_context,
                prefix=''
            )
    ...
    def _split_collision(self, context: CollisionContext) -> AlgorithmStepResult:
        ...
        if group_1:
            ...
            actual_size_1 = float(len(group_1))
            new_context_1 = CollisionContext(
                self.next_context_id, group_1, actual_size_1, new_prefix_1)
            self.next_context_id += 1
        ...
        if group_0:
            ...
            actual_size_0 = float(len(group_0))
            new_context_0 = CollisionContext(
                self.next_context_id, group_0, actual_size_0, new_prefix_0)
            self.next_context_id += 1
            priority = self._calculate_priority(
