...
MEAN_RESIDENCE_TIMES = [500, 750, 1000, 1250, 1500, 2000, 3000]


FIXED_SCENARIO_PARAMS = {
    "simulation_duration_s": 10.0,
    "tag_arrival_rate_per_s": 2000,
...
if __name__ == "__main__":
    ...
    tasks = []
    for res_time in MEAN_RESIDENCE_TIMES:
        current_scenario_params = FIXED_SCENARIO_PARAMS.copy()
        current_scenario_params['mean_residence_time_ms'] = res_time
    ...
    print(f"实验二: 鲁棒性评估 - 标签驻留时间的影响")
    print("-" * 60)
    print(f"总计仿真任务数: {len(tasks)} (已随机打乱)")
    ...
    output_directory = "exp2_results_vs_residence_time"

    analytics.save_to_csv(
        x_axis_key='mean_residence_time_ms',
    ...
    analytics.plot_results(
        ...
        save_path=os.path.join(output_directory, "exp2_summary_charts.png")
    )

    end_time = time.time()
    print("-" * 60)
    print(f"实验二全部完成，总耗时: {end_time - start_time:.2f} 秒。")
